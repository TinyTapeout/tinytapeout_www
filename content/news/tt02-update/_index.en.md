---
title: TT02 updates!
description: Design size doubled, slot count increased, major accessibility upgrade over TT01!
author: Tiny Tapeout Team
date: "2022-11-23"
type: news
tags: ['shuttle']
layout: single
cover:
    image: images/tt02-image.png
    alt: Tiny Tapeout 02
---

Big update for [TT02](/chips/tt02)! We've just rolled out changes to the size of designs.

250 slots, almost 2× bigger. As we've had fewer sales than expected, we've decreased the number of slots and increased the area each design gets. There are now 250 project slots, each 150×170 microns — almost 2× the area of before.

If you've already submitted and you're happy with your design, you don't need to do anything. All designs will be re-hardened before the final submission anyway. If you want to take advantage of the increased area, sync your repo's config.tcl with the new one in the [repository](https://github.com/TinyTapeout/tt02-submission-template/blob/main/src/config.tcl).

**Final submission deadline:** Friday 2nd December at 18:00 CET. 

Efabless updated the tapeout deadline to December 5th, but we're setting our own cutoff a few days earlier to allow time to prepare the final submission.

The [TT02 datasheet](/tt02.pdf) is already looking great, complete with a table of contents and a cover logo. There are some fantastic designs in there, including another FPGA from [Jan](https://github.com/grayresearch/tt02-s4ga) and five sound generators. 

{{< figure src="images/jan.png" title="FPGA from Jan" >}}

If you haven't filled in your docs yet, please do!

We're also asking TT01 submitters to merge their designs into TT02. If 50% of those designs get added, we'll be at around 140 designs, leaving 100 slots still to fill. 

If you know anyone interested in participating, especially schools or universities, send them our way → [tinytapeout.com](https://tinytapeout.com).

Looking forward to seeing your design in the submission soon!
