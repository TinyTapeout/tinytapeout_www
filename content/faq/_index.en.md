...
## How can I map an external clock to one of the GPIOs?

Tiny Tapeout provides the designers with a set of 24 general-purpose I/O pins: 8x are input-only (`ui_in`), 8x are output-only (`uo_out`) amd 8x are tri-state bi-directional (`uio_*`).
...
To be able to run this scenario we need to tweak the configuration file `config.tcl` that will be used by the Tiny Tapeoyt workflow, so that:
